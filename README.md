...
以下均为「晨雾」主题的 Android 真机完整长截图，没有裁切页面内容。
...
- 首页支持自定义常用上班时段、滑杆选择与数字快速输入，例如 `856 → 08:56`、`1230 → 12:30`。
- 完成上班打卡后，主卡片显示居中的预计下班时间、剩余时长和计算依据，并可随时修改打卡时间。
- 预计下班时间综合工作时长、实际午休、最近工作日午休或默认午休实时计算。
- 午休支持快速手动输入、`11:40～12:30` 时间滚轮、按当前时间打卡和 `0～90` 分钟固定午休。
- 今日与历史记录均可补录和修改；历史列表支持左滑删除，并提供周、月累计工时。
- 临近下班提醒仅通过 ClockOut 本机通知发送；应用内提示显示在右上角，可点叉或右滑关闭。
- 提供五套玻璃质感主题、三种字体、触觉反馈与 24 小时制选项。
- 内置机械工程公式、单位换算和科学计算器，所有数据与计算均保留在本机。
...
当前版本不包含登录、账户、云同步、团队协作、企业管理、公共目录导出、第三方统计或任何服务端能力。后续规划见 [docs/ROADMAP.md](docs/ROADMAP.md)。
...
- 上班打卡：可在设置中启用并自定义常用时间范围，首页输入与滑杆会同步使用该范围；关闭限制后可自由输入任意有效时间。支持 `830`、`856`、`1230` 等快速格式，当天不接受未来时间。
...
午休打卡默认支持开始、结束时间快速输入，输入 `1230` 会自动格式化为 `12:30`。也可在设置中切换为午休开始时间滚轮，范围为 `11:40～12:30`、默认定位 `12:00`。结束时间不得早于开始时间；午休未结束时，不生成错误的实际工作时长。
...
- 启用或关闭首页打卡时段限制，并自定义起止时间；首页滑杆自动同步。
...
- 应用内成功与错误提示显示在右上角，可点叉、从左向右滑动或等待自动关闭。
...
当前工程已通过单元测试、`lintDebug` 与 Debug APK 构建，并在 Android 真机完成安装和界面验证。ClockOut 前台请求设备可用的最高刷新率；最终刷新率仍取决于系统、省电和温控策略。
